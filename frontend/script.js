...
function renderComments(comments) {
  ...
  commentsList.innerHTML = comments.map(c => {
    const hasLiked = currentUserId && c.likes && c.likes.some(id => id === currentUserId || (typeof id === 'object' && id._id === currentUserId));
    const likesCount = c.likes ? c.likes.length : 0;
    
    let repliesHtml = '';
    if (c.replies && c.replies.length > 0) {
      repliesHtml = `<div style="margin-left: 24px; margin-top: 12px; display: flex; flex-direction: column; gap: 8px; border-left: 2px solid rgba(255,255,255,0.1); padding-left: 12px;">` + 
        c.replies.map(r => {
          const rHasLiked = currentUserId && r.likes && r.likes.some(id => id === currentUserId || (typeof id === 'object' && id._id === currentUserId));
          const rLikesCount = r.likes ? r.likes.length : 0;
          return `
            <div style="background: rgba(255,255,255,0.02); padding: 8px 12px; border-radius: 8px; position: relative;">
              <strong style="color: #a5b4fc; font-size: 0.8rem;">${r.user?.name || 'Unknown User'}</strong>
              <p style="margin: 4px 0; font-size: 0.85rem; color: #e2e8f0;">${r.text}</p>
              <div style="display: flex; gap: 12px; align-items: center;">
                <small style="color: #64748b; font-size: 0.7rem;">${new Date(r.createdAt).toLocaleDateString()}</small>
                <button onclick="toggleReplyLike('${c._id}', '${r._id}')" style="background: none; border: none; padding: 0; color: ${rHasLiked ? '#6366f1' : '#64748b'}; cursor: pointer; font-size: 0.8rem; display: flex; align-items: center; gap: 4px;">
                  ${rHasLiked ? '❤️' : '🤍'} ${rLikesCount}
                </button>
              </div>
            </div>
          `;
    ...
    return `
      <div style="background: rgba(255,255,255,0.03); padding: 12px; border-radius: 12px; position: relative;">
        <strong style="color: #a5b4fc; font-size: 0.9rem;">${c.user?.name || 'Unknown User'}</strong>
        <p style="margin: 6px 0; font-size: 0.95rem; color: #f8fafc;">${c.text}</p>
        
        <div style="display: flex; gap: 16px; align-items: center;">
          <small style="color: #64748b; font-size: 0.75rem;">${new Date(c.createdAt).toLocaleDateString()}</small>
          
          <button onclick="toggleCommentLike('${c._id}')" style="background: none; border: none; padding: 0; color: ${hasLiked ? '#6366f1' : '#94a3b8'}; cursor: pointer; font-size: 0.85rem; display: flex; align-items: center; gap: 4px;">
            ${hasLiked ? '❤️' : '🤍'} ${likesCount}
          </button>
          
          <button onclick="showReplyInput('${c._id}')" style="background: none; border: none; padding: 0; color: #94a3b8; cursor: pointer; font-size: 0.85rem; font-weight: 500;">
            Reply
          </button>
        </div>
        
        <div id="reply-input-${c._id}" style="display: none; margin-top: 10px; gap: 8px;">
          <input type="text" id="reply-text-${c._id}" placeholder="Write a reply..." style="flex: 1; padding: 8px 12px; border-radius: 8px; border: 1px solid rgba(255,255,255,0.1); background: rgba(0,0,0,0.2); color: white; font-size: 0.85rem;">
          <button onclick="submitReply('${c._id}')" class="btn btn-primary" style="padding: 8px 16px; font-size: 0.85rem;">Post</button>
        </div>
...
window.toggleCommentLike = async function(commentId) {
  ...
  try {
    const res = await fetch(`${getApiBaseUrl()}/api/projects/${currentOpenProjectId}/comment/${commentId}/like`, {
      method: "POST",
...
window.toggleReplyLike = async function(commentId, replyId) {
  ...
  const token = localStorage.getItem("authToken");
  try {
    const res = await fetch(`${getApiBaseUrl()}/api/projects/${currentOpenProjectId}/comment/${commentId}/reply/${replyId}/like`, {
      method: "POST",
      headers: { "Authorization": `Bearer ${token}` }
    });
    if (res.ok) {
...
window.submitReply = async function(commentId) {
  ...
  const token = localStorage.getItem("authToken");
  try {
    const res = await fetch(`${getApiBaseUrl()}/api/projects/${currentOpenProjectId}/comment/${commentId}/reply`, {
      method: "POST",
...
function openProjectModal(project) {
  ...
  if (downloadBtn) {
    if (project.fileUrl) {
      ...
      downloadBtn.onclick = async (e) => {
        ...
        try {
          const fileUrlPath = project.fileUrl.startsWith('/') ? project.fileUrl : `/${project.fileUrl}`;
          const res = await fetch(`${getApiBaseUrl()}${fileUrlPath}`);
          ...
          a.href = url;
          a.download = decodeURIComponent(fileUrlPath.split('/').pop());
  ...
  if (githubBtn) {
    if (project.githubLink) {
      githubBtn.href = normalizeUrl(project.githubLink);
  ...
  const deployBtn = document.getElementById("projectModalDeploy");
  if (deployBtn) {
    if (project.deployLink) {
      deployBtn.href = normalizeUrl(project.deployLink);
...
window.downloadProjectFile = async function(e, fileUrl) {
  e.preventDefault();
  ...
  try {
    const fileUrlPath = fileUrl.startsWith('/') ? fileUrl : `/${fileUrl}`;
    const res = await fetch(`${getApiBaseUrl()}${fileUrlPath}`);
    if (!res.ok) throw new Error("File not found");
    const blob = await res.blob();
    const url = window.URL.createObjectURL(blob);
  ...
  try {
    ...
    if (res.ok && result.success) {
      ...
      list.innerHTML = notifs.map(n => {
        ...
        let onClickAction = `closeNotificationModal(); openPublicProfile('${u._id}')`;
        if ((n.type === "like" || n.type === "comment") && n.project) {
          onClickAction = `closeNotificationModal(); openProjectById('${n.project._id}')`;
        }
        ...
        return `
          <li style="display: flex; flex-direction: column; padding: 16px; border-radius: 12px; background: ${bgColor}; border: 1px solid ${borderColor};">
            <div style="display: flex; align-items: center; gap: 12px; cursor: pointer;" onclick="${onClickAction}">
...
window.openProjectById = async function(projectId) {
  try {
    const res = await fetch(`${getApiBaseUrl()}/api/projects/${projectId}`);
    const result = await res.json();
    if (res.ok && result.success) {
      openProjectModal(result.data);
      // Optional: if it's a comment click, focus the comment input
      setTimeout(() => {
      ...
      notify("Project not found or deleted");
    }
  } catch (err) {
    notify("Network error loading project");
  }
}
  ...
  try {
    ...
    projects.forEach(p => {
      ...
      li.innerHTML = `
        ...
        <div class="project-actions" style="display: flex; gap: 0.75rem; flex-wrap: wrap;">
          ${p.githubLink ? `<a href="${normalizeUrl(p.githubLink)}" target="_blank" class="btn btn-secondary" style="padding: 10px 20px; font-size: 0.85rem; border-radius: 10px; display: inline-flex; align-items: center; justify-content: center;">GitHub</a>` : ''}
          ${p.deployLink ? `<a href="${normalizeUrl(p.deployLink)}" target="_blank" class="btn btn-secondary" style="padding: 10px 20px; font-size: 0.85rem; border-radius: 10px; display: inline-flex; align-items: center; justify-content: center;">Live</a>` : ''}
          ${p.fileUrl ? `<a href="#" onclick="downloadProjectFile(event, '${p.fileUrl}')" class="btn btn-secondary" style="padding: 10px 20px; font-size: 0.85rem; border-radius: 10px; display: inline-flex; align-items: center; justify-content: center;">Download</a>` : ''}
