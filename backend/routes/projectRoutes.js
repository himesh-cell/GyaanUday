...
router.post("/:id/comment/:commentId/like", protect, async (req, res, next) => {
  try {
    if (!mongoose.isValidObjectId(req.params.id) || !mongoose.isValidObjectId(req.params.commentId)) {
      return res.status(400).json({ success: false, message: "Invalid IDs" });
    }
...
// Reply to a comment
router.post("/:id/comment/:commentId/reply", protect, async (req, res, next) => {
  try {
    if (!mongoose.isValidObjectId(req.params.id) || !mongoose.isValidObjectId(req.params.commentId)) {
      return res.status(400).json({ success: false, message: "Invalid IDs" });
    }
    ...
    if (comment.user.toString() !== req.user._id.toString()) {
      await Notification.create({
        ...
        sender: req.user._id,
        type: "comment", // You can keep "comment" or add a new "reply" type later
        project: project._id
      });
    }
...
// Like/Unlike a reply
router.post("/:id/comment/:commentId/reply/:replyId/like", protect, async (req, res, next) => {
  try {
    if (!mongoose.isValidObjectId(req.params.id) || 
